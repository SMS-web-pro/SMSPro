...
export function NewCampaignPage() {
  ...
  const handleSend = async () => {
    setSending(true)
    try {
      // Étape 1 : Créer la campagne via l'API
      ...
      if (createResult.error || !createResult.data) {
        addToast({
          type: 'error',
          title: 'Erreur',
          description: createResult.error || 'Impossible de créer la campagne',
        })
        setSending(false)
        return
      }

      const campaign = createResult.data

      // Étape 2 : Si envoi immédiat, envoyer via Twilio
      if (form.sendType === 'now' && targetContacts.length > 0) {
        const contactIds = targetContacts.map((c) => c.id)
        const result = await sendSMSHook(contactIds, form.message, { campaignId: campaign.id })
        ...
        addToast({
          ...
          description: `${targetContacts.length} destinataires — envoi programmé`,
        })
      }
